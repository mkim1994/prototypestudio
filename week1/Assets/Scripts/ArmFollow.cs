...
public class ArmFollow : MonoBehaviour {
    ...
    public Transform noashands;
    private Vector3 noashandsPos;
    private bool noahandback;
	...
	void Start () {
        ...
        noashandsPos = noashands.position;
        noahandback = false;
	}
	...
	void Update () {
        ...
        else
        {
            ...
            if(noahandback){
                noahandback = false;
                noashands.DOMove(noashandsPos, 2f);
            }
        }


        if (!handBackOff && !Services.GameManager.dateSuccess)
        {
            mousePosition += new Vector3(Input.GetAxis("Mouse X")*0.2f,
                                         Input.GetAxis("Mouse Y")*0.2f, 0);
